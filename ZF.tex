...
\begin{itemize}
    ...
    \item EtherCAT (nicht TCP basierend)
    \item Ethernet/IP
\end{itemize}

\newcolumn

\section{Diskrete Regelungsaufgabe}
\textnormal{Eine diskrete Regelungsaufgabe benötigt Zeit. Über Messung der Sensorwerte, Vergleich mit Sollwert etc. verzögern die Reaktionszeit des Reglers drastisch.}

\begin{figure}[H]
    \centering
    \includegraphics[width =  0.8\columnwidth]{sc/diskreterRegler.pdf}
\end{figure}

\textnormal{Es gilt dabei die folgende Bedingung:}
\begin{equation*}
    T_{AD} + T_C + T_{DA} << T
\end{equation*}
...
\end{itemize}
\textnormal{Ingesamt wären es 6, da noch IL (Instruction List) definiert wäre.}
\end{tcolorbox}
...
\begin{table}[H]
    ...
    \begin{tabular}{| p{0.7cm} | p{1.6cm} | p{1.9cm}| p{0.6cm} | p{2cm}|}
        ...
        \mintinline{C}{TIME}&\mintinline{text}{T#0ms}&\mintinline{text}{T#71582m}&32&\\
        &&\mintinline{C}|47s295ms|&&\\\hline
        \mintinline{C}{TOD}&\mintinline{text}{TOD#00:00}&\mintinline{text}{TOD#1193:02:}&32&\\
        &&\mintinline{C}{47.295}&&\\\hline
        \mintinline{C}{DATE}&\mintinline{text}{D#1970-01-01}&\mintinline{text}{D#2106-02-06}&32&\\\hline
        \mintinline{C}{DT}&\mintinline{text}{DT#1970-0-0}&\mintinline{text}{DT#2106-02-06-}&32&\\
...
\begin{tcolorbox}[colback=VeryLightOrange,colframe=DarkOrange,title=Conditional Operatoren]
\textnormal{Wie in C gibt es in ST mehrere Conditional Operatoren. Diese sind wiefolgt:}

\begin{table}[H]
    \centering
    \tabularcount
    \begin{tabular}{l l | l l}
        \mintinline{text}{=} & ist gleich & \mintinline{text}{AND} & und\\
        \mintinline{text}{<>} & ist ungleich & \mintinline{text}{OR} & oder\\
        \mintinline{text}{<} & kleiner als & \mintinline{text}{NOT} & 'nicht'\\
        \mintinline{text}{>} & grösser als & \mintinline{text}{XOR} & exklusiv oder\\
        \mintinline{text}{<=} & kleiner gleich&\\
        \mintinline{text}{>=} & grösser gleich&
    \end{tabular}
\end{table}

\textnormal{Diese Operatoren können kombiniert angewendet werden:}
\begin{lstlisting}[]
IF motor AND NOT fault THEN
IF (a = b) AND (x <> y OR NOT z) THEN
\end{lstlisting}
\end{tcolorbox}
